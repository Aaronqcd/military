...
(function() {
    window.RL_YTX = window.RL_YTX || {
        ...
        init: function(appid, connecterIp, connecterPort, fileServerIp, fileServerPort) {
		    ...
		    this.RegisterCallBack(ECSDK, "onDownloadFileComplete", onDownloadFileListener_ytx);//文件下载通知          
            this.RegisterCallBack(ECSDK, "onRecordingTimeOut", onAudioMakeLisener_ytx); //录音监听事件           
            this.RegisterCallBack(ECSDK, "onFinishedPlaying", onAudioPlayLisener_ytx);//放音监听事件
            //voip事件
			...
			if(typeof remoteView != "undefined" && typeof localView!="undefined"){
            ...
            return result;
            /*ECSDK.SetCodecEnabled(7, false);*///因为录像只支持h264,需要禁用vp8编码，
        },
