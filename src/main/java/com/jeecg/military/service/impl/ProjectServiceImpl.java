...
public class ProjectServiceImpl extends CommonServiceImpl implements ProjectServiceI {
	...
	private void doDelBus(ProjectEntity t) throws Exception{
 	...
 	private Map<String,Object> populationMap(ProjectEntity t){
		...
		map.put("show_pic", t.getShowPic());
		map.put("project_intro", t.getProjectIntro());
		return map;
	}
	 ...
	 */
 	public String replaceVal(String sql,ProjectEntity t){
 		...
 		sql  = sql.replace("#{show_pic}",String.valueOf(t.getShowPic()));
 		sql  = sql.replace("#{project_intro}",String.valueOf(t.getProjectIntro()));
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("project",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
